 ...
 *   Function|Brief
 *   ---|---
 *   getGitRepoName()	| Get name of current repo `.git`.
...
function getGitHistory( $branch = 'main' )
{
	...
	foreach( $data as $entry)
	{
		...
		*/
        ...
        // 'commit' or 'commit (initial)'
		preg_match('/^(\w+) (\w+) (.*?) (\d+) ([+-]*\d+)\t([\w\(\) ]+): (.*?)$/', $entry, $matches);
		$history[]	= [
			'from'		=> $matches[1] ?? '?',
			'to'		=> $matches[2] ?? '?',
			'committer'	=> $matches[3] ?? '?',
			'date'		=> date('Y-m-d H:i:s', (int)($matches[4] ?? 0 )) . ( $matches[5]  ?? '?' ),
			'action'	=> $matches[6] ?? '?',
			'comment'	=> $matches[7] ?? '?',
		];
	}
 ...
 */
function getVersion($level = 0, $arr = FALSE )
{
	$lifecycle	= [
		'π'	// Pró or Pre-alpha refers to all activities performed during the software project before formal testing.
	,	'α'	// Alpha software is not thoroughly tested by the developer before it is released to customers.
	...
	// Get version from `version.txt`: v.0.0.1-alpha
	$gitVersion		= trim( @file_get_contents( ( $GLOBALS['releaseroot'] ?? __DIR__.'/../' ) . 'version.txt' ) );
    ...
    if ( $arr )
    {
        ...
        $gitVersion = str_replace( ['.','-'], '_', $gitVersion );
        // Remove prefix 'v'
        $gitVersion = ltrim( $gitVersion, 'v' );
        //$gitVersion = substr( $gitVersion, 1 );
        $gitVersion = explode( '_', $gitVersion );
    }
	return( $gitVersion );
}	// getVersion()
